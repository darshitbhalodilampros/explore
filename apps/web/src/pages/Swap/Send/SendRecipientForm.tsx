...
const AutocompleteRow = ({
  ...
  const boundSelectRecipient = useCallback(
    ...
    [address, "", selectRecipient, unitag?.username],
  );
...
export function SendRecipientForm({ disabled }: { disabled?: boolean }) {
  ...
  const unitagMetadata = useUnitagByNameWithoutFlag(
  ...
  const handleFocus = useCallback(
    (focus: boolean) => setFocus([focus, false]),
    [],
  );
  const handleForceFocus = useCallback(
    (focus: boolean) => setFocus([focus, true]),
    [],
  );
  ...
  useOnClickOutside(
    inputWrapperNode,
    isFocusing ? () => handleFocus(false) : undefined,
  );
      ...
      {showInputField ? (
        ...
        <StyledConfirmedRecipientRow>
          <StyledConfirmedRecipientDisplayRow
            gap="md"
            onClick={editValidatedRecipient}
          >
